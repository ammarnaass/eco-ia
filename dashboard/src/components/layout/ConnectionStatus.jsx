...
export default function ConnectionStatus() {
  ...
  return (
    <div className="hidden xl:flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800">
      ...
      {channels.map(ch => (
        <div key={ch.key} className="flex items-center gap-0.5" title={`${ch.label}: ${channels[ch.key] ? 'مفعل' : 'متوقف'}`}>
          <span className={`w-1.5 h-1.5 rounded-full ${channels[ch.key] ? ch.color : 'bg-slate-300 dark:bg-slate-600'}`}></span>
        </div>
      ))}
    </div>
  )
